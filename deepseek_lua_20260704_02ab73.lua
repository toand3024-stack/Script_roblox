--[[
    FPS GOD MENU v16 ULTIMATE – TOANDINH STYLE UI (FIXED FOR DELTA + MOBILE)
    - Tất cả tab đều có thanh cuộn để không bị che mục bên dưới
    - Sit on Head chuyển sang danh sách cuộn như Teleport
    - Tối ưu hiệu năng cho mobile yếu (giảm tần suất update)
]]
...
-- ===================== ANTI-BAN (CHỈ DÙNG HOOKFUNCTION) =====================
...
local function arrange(container)
    ...
    return yPos + 5  -- trả về chiều cao tổng
end
...
-- Tạo các tab frame (chiều cao sẽ được set sau)
for i, tab in ipairs(TabList) do
    local f = Instance.new("Frame")
    f.Size = UDim2.new(1,0,0,0)  -- chiều cao động
...
-- Hàm chuyển tab và cập nhật CanvasSize
local function switchTab(index)
    ...
    frame.Visible = true
    -- Cập nhật CanvasSize theo chiều cao của tab hiện tại
...
-- Thay dropdown Sit on Head bằng ScrollingFrame danh sách player
...
-- ToggleFly, ToggleNoclip, HandleFly
...
-- ===================== VÒNG LẶP CHÍNH (TỐI ƯU CHO MOBILE) =====================
local FOVCircle
...
print("[FPS GOD ULTIMATE v16 – FIXED + MOBILE OPTIMIZED] Đã tải thành công!")