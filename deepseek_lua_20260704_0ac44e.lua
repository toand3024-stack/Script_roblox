--[[
    FPS GOD MENU v16 ULTIMATE – TOANDINH STYLE UI (FIXED SCROLLABLE SIT LIST)
    - Sit on Head: danh sách người chơi dạng cuộn (giống Teleport)
    - Fake Lag nâng cấp (max 2000ms, status label)
...
CreateSwitch(trollFrame, "Fake Lag", false, function(v)
    ...
    if v then
        lagStatusLabel.Text = "Status: ON ("..Settings.FakeLagIntensity.."ms)"
    else
        lagStatusLabel.Text = "Status: OFF"
    end
end)

CreateSlider(trollFrame, "Lag Intensity", 50, 2000, 200, function(v)
    Settings.FakeLagIntensity = v
    if Settings.FakeLag then
        NetworkClient:SetFakeLatency(v/1000)
        lagStatusLabel.Text = "Status: ON ("..v.."ms)"
    else
        lagStatusLabel.Text = "Status: OFF"
    end
end)

local lagStatusLabel = Instance.new("TextLabel")
lagStatusLabel.Size = UDim2.new(1, -20, 0, 24)
lagStatusLabel.Position = UDim2.new(0, 10, 0, 0)
lagStatusLabel.BackgroundTransparency = 1
lagStatusLabel.Text = "Status: OFF"
lagStatusLabel.Font = Enum.Font.GothamBold
lagStatusLabel.TextSize = 14
lagStatusLabel.TextColor3 = Color3.fromRGB(200, 200, 200)
lagStatusLabel.TextXAlignment = Enum.TextXAlignment.Left
lagStatusLabel.Parent = trollFrame
...
-- Sit on Head (dùng ScrollingFrame có thể cuộn)
...
sitPlayerListFrame.Size = UDim2.new(1, -20, 0, 120)  -- chiều cao cố định để cuộn
...
print("[FPS GOD ULTIMATE v16 – SCROLLABLE SIT LIST] Đã tải thành công!")