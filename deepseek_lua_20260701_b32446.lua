--[[
    FPS GOD MENU v11 – MOBILE (DELTA EXECUTOR)
    * Menu mặc định to (420x600), có thể kéo resize góc phải dưới.
    * Nút ẩn (━) và đóng (✕) to hơn (40x40).
    * Nút mở (⚡) sau khi ẩn menu có thể kéo thả đến vị trí bất kỳ.
    * Vòng FOV hiển thị rõ nét (viền dày 2.5px).
    * Tất cả chức năng: Aimbot, ESP, Fly, Noclip, Teleport, Fake Lag, Chat Spam...
...
local scaleFactor = math.min(viewport.X / 420, viewport.Y / 680)
local UIScale = Instance.new("UIScale")
UIScale.Scale = scaleFactor
UIScale.Parent = ScreenGui

local MainFrame = Instance.new("Frame")
MainFrame.Size = UDim2.new(0, 420, 0, 600)   -- to hơn mặc định
...
-- Nút mở menu (ẩn ban đầu, có thể kéo)
...
UIS.InputChanged:Connect(function(input)
    if resizing and (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) then
        ...
        local newWidth = math.clamp(startSize.X.Offset + delta.X / UIScale.Scale, 320, 600)
...
MinimizeBtn.MouseButton1Click:Connect(function()
    ...
    -- đặt vị trí nút mở gần vị trí menu
...
for i, name in ipairs(tabs) do
    ...
    content.Size = UDim2.new(1, -6, 1, -92)
    content.Position = UDim2.new(0, 3, 0, 88)
...
local function CreateToggle(parent, name, default, callback)
    ...
    indicator.Size = UDim2.new(0, 44, 0, 44)
    indicator.Position = UDim2.new(0.85, 0, 0.5, -22)
...
local function CreateSlider(parent, name, min, max, default, callback)
    ...
    knob.Position = UDim2.new((default - min) / (max - min), -16, 0.5, -16)
    ...
    UIS.InputChanged:Connect(function(input)
        if draggingSlider and (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) then
            ...
            knob.Position = UDim2.new(alpha, -16, 0.5, -16)
...
playerListFrame.Size = UDim2.new(1, -10, 0, 220)
...
msgInput.Size = UDim2.new(1, -10, 0, 44)
...
FOVCircle.Thickness = 2.5    -- dày hơn để dễ thấy
...
print("[FPS GOD MENU v11] To hơn, nút kéo mở, resize, FOV đã hiện, Anti-Ban mạnh mẽ.")